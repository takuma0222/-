...
将来的には `WriteInfoLog` メソッドを追加し、成功ログと失敗ログを分離することを推奨します。

### 5. 同時実行制御

#### 重複検索の防止
- **実装方法**: `_lastEmployeeSearchNo` フィールドで最後に検索した従業員NOを記録
- **動作**: 同じ従業員NOで連続して検索を実行しない（TextChangedイベントの重複呼び出し対策）

#### ファイル読み込みの排他制御
- **実装方法**: `SemaphoreSlim` による排他制御
- **動作**: 複数のスレッドが同時に `LoadAsync` を呼び出しても、1回だけファイル読み込みを実行
- **利点**: 高速入力時のファイル多重読み込みを防止
